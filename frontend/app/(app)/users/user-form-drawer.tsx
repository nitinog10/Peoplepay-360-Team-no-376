...
const assignableRole = z.enum(["EMPLOYEE", "HR_MANAGER", "HR_PAYROLL_USER", "HR_PAYROLL_MANAGER", "ADMIN"]);
...
export function UserFormDrawer({ user, trigger }: { user?: ManagedUser; trigger: React.ReactNode }) {
  ...
  const form = useApiForm<Values, ManagedUser>({
    ...
    fields: user ? ["username", "password", "role", "isActive"] : ["employeeId", "username", "password", "role", "isActive"],
    submit: (values) => {
      if (!user) {
  ...
  return (
    <Sheet open={open} onOpenChange={(next) => { setOpen(next); form.setFormError(null); if (next) form.form.reset(defaults(user)); }}>
      <SheetTrigger asChild>{trigger}</SheetTrigger>
      <SheetContent>
        <SheetHeader>
          <SheetTitle>{user ? `Edit ${user.username}` : "Create user account"}</SheetTitle>
          <SheetDescription>
        ...
        <Form api={form}>
          ...
          {user && (
            <div className="rounded-lg border bg-muted/30 px-3 py-2">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Linked employee</p>
              <p className="mt-1 font-medium">{user.employee.firstName} {user.employee.lastName}</p>
              ...
              <Field name="role" label="Role" required>
                {(control) => (
                  <Select value={selectedRole} onValueChange={(value) => form.form.setValue("role", value as AssignableRole, { shouldDirty: true, shouldValidate: true })}>
                    ...
                    <SelectContent>
                      {(roles.data?.data ?? []).map((role) => <SelectItem key={role.roleId} value={role.roleName}>{formatRoleName(role.roleName)}</SelectItem>)}
                    </SelectContent>
                  </Select>
                )}
              </Field>
