...
- `EMPLOYEE` reads are scoped to the caller's employee record.
- HR-capable roles can access HR rows according to their permissions.
- `HR_PAYROLL_USER` adds payroll processing and read-only salary configuration.
- `HR_PAYROLL_MANAGER` adds configuration write and permitted deletion capabilities.
- `ADMIN` receives all code-defined permissions and unrestricted row scope.
- Non-admin users with `users:manage` may manage `EMPLOYEE` accounts only.
- `GET /roles/permissions` is ADMIN-only and returns the read-only code-defined matrix.

Deactivation and password changes revoke refresh sessions. An already-issued access token remains valid until it expires.
